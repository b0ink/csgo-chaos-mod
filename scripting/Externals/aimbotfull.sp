...
bool g_cvFlashbang = true;
// bool g_cvShowText = true;
...
public Action Aimbot_Event_WeaponFire(Event hEvent, const char[] chName, bool bDontBroadcast)
{
...
stock int GetClosestClient(int iClient)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			...
			if (g_cvFlashbang && g_bFlashed[iClient])
			{
				continue;
			}
